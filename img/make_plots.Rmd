...
```{r}
devtools::install_github("peterdeweirdt/ggpointdensity")
```
...
ggsave(here("img/scatter_dens_bin2d.png"),
...
ggsave(here("/img/pointdensity.png"),
       width = 10, height = 7, units = "cm", dpi = 300)
```

Alter the stat to kde:
```{r}
dat %>%
  ggplot(aes(x = x, y = y)) +
  geom_pointdensity(size = .3, stat = 'pointkde') +
  scale_color_viridis() +
  labs(title="geom_pointdensity(stat = )") + 
...
ggsave(here("/img/pointdensity_logo.png"),
...
adj1 + adj2
ggsave(here("/img/pointdensity_adj.png"),
...
ggsave(here("/img/pointdensity_xyadj.png"),
       width = 10, height = 7, units = "cm", dpi = 300)
```
...
ggsave(here("/img/pointdensity_facet.png"),
...
```{r}
plim + pcc
ggsave(here("/img/pointdensity_zoom.png"),
       width = 18, height = 6, units = "cm", dpi = 300)
```
...
ggsave(here("/img/pointdensity_shape.png"),
       width = 10, height = 7, units = "cm", dpi = 300)
```
...
ggsave(here("/img/pointdensity_custom.png"),
